        ...
        try:
            ...
            for entry in quote_id_list:
                ...
                if self.args.until_now:
                    sql = "select * from usastock_dd where StockID = '%s' order by DDate desc" % (
                        ticker)
                    with conn.cursor() as cursor:
                ...
                self.logger.debug('== upate (%s, %s) period (%s, %s) ==' % (ticker, name,
                                                            start_date.strftime('%Y%m%d'),
                                                            end_date.strftime('%Y%m%d')))
                quotes = yahoo_quote.get_quote(ticker, start_date.strftime('%Y%m%d'), end_date.strftime('%Y%m%d'))
